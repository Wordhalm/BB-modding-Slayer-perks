this.rip_and_tear <- this.inherit("scripts/skills/skill", {
	m = {},
	function create()
	{
		this.m.ID = "actives.rip_and_tear";
		this.m.Name = "Rip and Tear";
		this.m.Description = "Succomb to your inner beast. Rip. And. Tear. On kill, regenerates 10% of the maximum of your hitpoints each turn for 2 turns.";
		this.m.KilledString = "Ripped to shreds";
		this.m.Icon = "skills/rip_and_tear.png";
		this.m.IconDisabled = "skills/rip_and_tear.png";
	...
	function getTooltip()
	{
		...
		if (this.getContainer().hasSkill("background.wildman") || this.getContainer().hasSkill("background.brawler") || this.getContainer().hasSkill("background.legend_commander_berserker") || this.getContainer().hasSkill("background.legend_berserker") || this.getContainer().hasSkill("background.legend_druid_commander") || this.getContainer().hasSkill("background.legend_druid"))
		{
	...
	function onAnySkillUsed( _skill, _targetEntity, _properties )
	{
		if (_skill == this)
		{
			...
			if (this.getContainer().hasSkill("background.wildman") || this.getContainer().hasSkill("background.brawler") || this.getContainer().hasSkill("background.legend_commander_berserker" || this.getContainer().hasSkill("background.legend_berserker")))
			{
				damageMin = damageMin * 1.25;
				damageMax = damageMax * 1.25;
			}
	...
	{
		// this.Tactical.EventLog.log(_skill);
		// this.Tactical.EventLog.log(this.getContainer().getSkillByID("effects.sated"));
		if (this.getContainer().hasSkill("effects.sated") && _skill == this.getContainer().getSkillByID("actives.rip_and_tear"))
  			this.getContainer().getSkillByID("effects.sated").effectProc();
	}
